...
public class GameTree {
    ...
    private TreeNode readTree(java.io.BufferedReader reader) throws java.io.IOException {
        ...
        //if it is a node.leaf, sets the node value
        if (node.leaf) {
            char v = s.charAt(2);
     ...
     */
    int findValue() {
        return findValueForNode(root, true);
    }

    private int findValueForNode(TreeNode node, boolean turn) {
        if (node == null) {
    ...
    public static void main(String[] args) {
        GameTree misere = new GameTree();
        misere.readTree("variants/misere.txt");
        System.out.println("misere winner: " + misere.findValue());

        GameTree arbitrary = new GameTree();
        arbitrary.readTree("variants/arbitrary.txt");
        System.out.println("arbitrary winner: " + arbitrary.findValue());

        GameTree notie = new GameTree();
        notie.readTree("variants/notie.txt");
        System.out.println("notie winner: " + notie.findValue());
