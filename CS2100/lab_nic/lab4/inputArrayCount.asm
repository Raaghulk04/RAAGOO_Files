...
main:
	...
	# code to read user input values and store into arrayA
	addi $t1, $zero, 8	# stores the number of read values
read:
	beq $t1, $zero, stop # stop when loop done
	li $v0, 5 			# system call code for read_int
	syscall	
	sb $v0, 0($t0)		# store integer to array
	addi $t0, $t0, 4	# ptr++
	addi $t1, $t1, -1	# read_count--;
	j read
stop:
	# reset ptr to arrayA
	la $t0, arrayA
