...
class BankSimulation extends Simulation {
  ...
  public BankSimulation(Scanner sc) {
    ...
    int l = sc.nextInt(); // max len counter queue
    int m = sc.nextInt(); // max len bank queue
    Bank bank = new Bank(k, m, l);
    ...
    for (int i = 0; i < n; i++) {
      ...
      Customer customer = new Customer(arrivalTime, serviceTime, task, money);
